...
export default function Home() {
  ...
  const sorteWeekDays = weekDays.slice(todayWeekDay + 1).concat(weekDays.slice(0, todayWeekDay + 1))
  ...
  return (
   <main className="container relative flex flex-col gap-8 px-4 pt-16">
    ...
    Object.entries(habits).map(([habit, habitStreak]) => (
      <div key={habit} className="flex flex-col gap-2">
        <div className="flex justify-between items-center text-white">
          <span className="text-xl font-light font-sans">
            {habit}
            </span>
          <button>
            <Image 
    ...
    text-neutral-900 bg-[#fff] font-display font-regular text-2xl p-2
    rounded-md" 
    >
      Novo hábito
   </Link>
